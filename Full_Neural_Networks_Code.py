...
class DNN_2layers:
    def train(self,X,y,epochs,learning_rate=1,decay=1e-2,momentum = 0.5,eps=1e-7,optim='AdaGrad',rho=0.9,beta_1=0.9,beta_2=0.999):
        ...
        if optim == 'SGD':
            ...
            optimizer = Optimizer_RMSprop(learning_rate,decay,eps,rho)
        elif optim == 'Adam':
            optimizer = Optimizer_Adam(learning_rate,decay,eps,beta_1,beta_2)
        else :
            optimizer = Optimizer_AdaGrad(learning_rate,decay,eps)
...
predictions = DNN_2layers().train(X,y,10001,learning_rate=0.05,decay = 5e-7,optim='Adam')
